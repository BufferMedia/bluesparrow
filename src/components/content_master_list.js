...
class ContentMasterList extends React.Component {
  ...
  loadContent = async => {
    try {
      axios
        .get("content", axios.getHeaders())
        .then(resp => {
          console.log(resp.data.data);
          this.setState({
            contentList: resp.data.data.map(content => {
              let category = "",
                contentType = "";
              axios.getCategories().forEach(e => {
                if (e.id === content.category) {
                  category = e.name;
                }
              });
              axios.getContentType().forEach(e => {
                if (e.id === content.content_type) {
                  contentType = e.name;
                }
              });
              return {
                    ...
                    <ul className="m-subheader__breadcrumbs m-nav m-nav--inline">
                      ...
                      <li className="m-nav__item">
                        <a href="" className="m-nav__link">
                          <span className="m-nav__link-text">List Content</span>
                        </a>
                      </li>
                      <li className="m-nav__separator">-</li>
                    </ul>
                  ...
                  <div className="m-portlet__head">
                    <div className="m-portlet__head-caption">
                      <div className="m-portlet__head-title">
                        <h3 className="m-portlet__head-text">List Content</h3>
                      </div>
                    </div>
